...
class HuffmanCoding:
    ...
    def __decodeText(self,text):
        ...
        for bit in text:
            currrent_bits += bit
            if currrent_bits in self.__revCodes:
                decoded_text += self.__revCodes[currrent_bits]
                currrent_bits = ''
    ...
    def decompress(self,input_path):
        filename, filextension = os.path.splitext(self.path)
        output_path = filename + "_decompressed" + ".txt"
        with open(input_path,'rb') as file, open(output_path,'w') as output:   #rb = read binary  
            bit_string = ""
            byte = file.read(1)
            while byte:
                byte = ord(byte)
                bits = bin(byte)[2:].rjust(8,'0')   #this is important bin(byte) will give b' 00011 something if width is not 8 then rest will give 0s appended to it before
                bit_string += bits
                byte = file.read(1)
...
path = '/Volumes/Personal/DSA/22HuffmanCoding/sample.txt'
